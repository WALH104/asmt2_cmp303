SingleLinkedList<T> is a singly linked list that uses an internal Node<T> class to store data and link to the next node. The list keeps track of a head, tail, and size to manage elements. If any get or remove method is called on an empty list, a NoSuchElementException is thrown.
